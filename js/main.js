...
(function () {
    ...
    function setupHandlers() {
        ...
        $('.webr-feedlist').on('click', '.webr-feeditem a', function (e) {
            ...
            renderArticleList(selectedFeed);
    ...
    function renderArticleList(fid) {
        ...
        if (cont && cont.length > 0) {
            for (var i in cont) {
                ...
                $('<td class="webr-article-title"></td>').html(cont[i].title).appendTo($tr);
                $('<td class="webr-article-date"></td>').html(moment(cont[i].date).format('DD MMM YYYY hh:mm:ss a')).appendTo($tr);
